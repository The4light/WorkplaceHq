...
import { Download, FileText, Wrench, BarChart2, BookOpen, X, Loader2 } from 'lucide-react'
import { supabase } from '../../lib/supabase'

// Helper map for dynamic category icons
...
export default function WHQResources() {
  ...
  const handleDownload = (e: React.FormEvent) => {
    e.preventDefault()
    if (!selectedResource?.file_url) return
  ...
  return (
    <div style={{ backgroundColor: 'var(--whq-bg)', minHeight: '100vh', fontFamily: 'var(--font-body)' }}>
      ...
      {/* Download Modal */}
      {selectedResource && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4" style={{ backgroundColor: 'rgba(0,0,0,0.5)', backdropFilter: 'blur(4px)' }} onClick={() => setSelectedResource(null)}>
          <div className="w-full max-w-md p-8 rounded-2xl shadow-2xl relative" style={{ backgroundColor: '#FFFFFF' }} onClick={e => e.stopPropagation()}>
            <div className="flex justify-between items-center mb-4">
              ...
              <button onClick={() => setSelectedResource(null)} className="p-1 rounded-lg hover:bg-gray-100">
                <X className="w-4 h-4" style={{ color: '#9CA3AF' }} />
              </button>
            </div>
            <p className="text-xs leading-relaxed mb-6" style={{ color: '#6B7280' }}>
              Enter your details to receive instant access to <strong style={{ color: '#0B3C2D' }}>{selectedResource.title}</strong>.
            </p>
            <form onSubmit={handleDownload} className="flex flex-col gap-3">
              ...
              <button 
                type="submit"
                className="w-full py-3 rounded-xl font-bold text-xs uppercase tracking-wider transition-opacity hover:opacity-95 mt-2" 
